...
const BirthdayWidget = () => {
  ...
  const handleWishHappyBirthday = (name: string) => {
    console.log(`Wishing happy birthday to ${name}`);
  };
          ...
          {todaysBirthdays.map((birthday) => (
            <div key={birthday.id} className="flex items-center space-x-3 p-2 bg-pink-50 rounded-lg">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-pink-600">Today is their birthday! 🎉</p>
              </div>
              <Button
                size="sm"
                onClick={() => handleWishHappyBirthday(birthday.name)}
                className="bg-pink-500 hover:bg-pink-600 text-white text-xs h-7 px-3"
              >
                Wish
              </Button>
            </div>
